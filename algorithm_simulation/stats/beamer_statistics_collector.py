    ...
    def conclusion(self):
        ...
        with open("./stats/report.csv",'ab+') as f:
            ...
            aa.writerow([str(self.model), str(self.connection_target), str(self.endtime), str(self.server_number), "None",
                         str(self.bucket_number),"None", str(self.total_con), "None", "None", str(self.max_imba), str(self.ave_imba)])
            f.close()
...
class DynamicBeamerCollecter(Collector):
    ...
    def conclusion(self):
        with open("./stats/report.csv", 'ab+') as f:
            aa = csv.writer(f)
            aa.writerow([str(self.model), str(self.connection_target), str(self.endtime), str(self.server_number), "None",
                         str(self.bucket_number), str(self.Imbalance_threshold), str(self.total_con), str(self.unnecessary_move_count),
                         str(float(self.unnecessary_move_count) / self.total_con), "None", "None", str((self.moving_bucket)/float(self.bucket_number))])
            f.close()
        self.moving_bucket = 0.0
        ...
        if self.change_time == 30000:
            ...
            with open("./stats/report.csv",'ab+') as f:
                ...
                aa.writerow([str(self.model), str(self.connection_target), str(self.endtime), str(self.server_number),
                             str(self.rate_number),"None", "None", str(self.total_con),str(self.unnecessary_move_count),str(float(self.unnecessary_move_count)/self.total_con), "None", "None"])
                f.close()
